...
const PublishFreight = () => {
  ...
  const searchMunicipalities = async (query: string, setOptions: (options: {
    name: string;
    state: string;
  }[]) => void) => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="w-80 p-0">
                        <div className="p-2">
                          <Input placeholder="Digite para pesquisar municípios" value={originSearch} onChange={e => {
                          setOriginSearch(e.target.value);
                          searchMunicipalities(e.target.value, setOriginOptions);
                        }} />
                        </div>
                        <div className="max-h-72 overflow-y-auto">
                          {originOptions.map(option => <div key={`${option.name}-${option.state}`} className="px-2 py-1 cursor-pointer hover:bg-gray-100" onClick={() => {
                  ...
                  <div>
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="w-80 p-0">
                        <div className="p-2">
                          <Input placeholder="Digite para pesquisar municípios" value={destinationSearch} onChange={e => {
                          setDestinationSearch(e.target.value);
                          searchMunicipalities(e.target.value, setDestinationOptions);
                        }} />
                        </div>
                        <div className="max-h-72 overflow-y-auto">
                          {destinationOptions.map(option => <div key={`${option.name}-${option.state}`} className="px-2 py-1 cursor-pointer hover:bg-gray-100" onClick={() => {
                    ...
                    <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className="w-full justify-start text-left font-normal">
                          {formData.loading_date ? format(formData.loading_date, "dd/MM/yyyy", {
                          locale: ptBR
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar mode="single" selected={formData.loading_date || undefined} onSelect={date => {
                        handleChange("loading_date", date);
                        setIsCalendarOpen(false);
                      }} disabled={date => date < new Date()} locale={ptBR} />
                      </PopoverContent>
                    </Popover>
                  </div>
                  ...
                  <div>
                    ...
                    <Popover open={isDeliveryDateCalendarOpen} onOpenChange={setIsDeliveryDateCalendarOpen}>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className="w-full justify-start text-left font-normal">
                          {formData.expected_delivery_date ? format(formData.expected_delivery_date, "dd/MM/yyyy", {
                          locale: ptBR
                        }) : <span>Selecione uma data</span>}
                          <ChevronDown className="ml-auto h-4 w-4 opacity-50" />
                        </Button>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar mode="single" selected={formData.expected_delivery_date || undefined} onSelect={date => {
                  ...
                  <div>
                    ...
                    <Input id="sender_company" placeholder="Ex: Transportadora ABC" value={formData.sender_company} onChange={e => handleChange("sender_company", e.target.value)} />
                  </div>

                  <div>
                    <Label htmlFor="cargo_type">Tipo de Carga *</Label>
                    <Select value={formData.cargo_type} onValueChange={value => handleChange("cargo_type", value)} required>
                      <SelectTrigger id="cargo_type">
                        <SelectValue placeholder="Selecione o tipo de carga" />
                      </SelectTrigger>
                      <SelectContent>
                        {cargoTypes.map(type => <SelectItem key={type} value={type}>
                            {type}
                          </SelectItem>)}
                      </SelectContent>
                    </Select>
                  ...
                  <div>
                    ...
                    <Input id="cargo_content" placeholder="Ex: Arroz, Computadores, etc." value={formData.cargo_content} onChange={e => handleChange("cargo_content", e.target.value)} />
                  </div>

                  <div>
                    <Label htmlFor="truck_type">Tipo de Caminhão/Carroceria *</Label>
                    <Select value={formData.truck_type} onValueChange={value => handleChange("truck_type", value)} required>
                      <SelectTrigger id="truck_type">
                        <SelectValue placeholder="Selecione o tipo de caminhão" />
                      </SelectTrigger>
                      <SelectContent>
                        {truckTypes.map(type => <SelectItem key={type} value={type}>
                            {type}
                          </SelectItem>)}
                      </SelectContent>
                    </Select>
                  ...
                  <div>
                    ...
                    <Input id="value" type="number" placeholder="Ex: 3500 (deixe em branco para 'A combinar')" value={formData.value} onChange={e => handleChange("value", e.target.value)} />
                  ...
                  <div>
                    ...
                    <Input id="contact" placeholder="Ex: (11) 98765-4321" value={formData.contact} onChange={e => handleChange("contact", e.target.value)} required />
                  </div>

                  <div>
                    <Label htmlFor="weight">Peso da Carga (kg)</Label>
                    <Input id="weight" type="number" placeholder="Ex: 1500" value={formData.weight} onChange={e => handleChange("weight", e.target.value)} />
                  </div>

                  <div>
                    <Label htmlFor="freight_distance">Distância do Frete (km)</Label>
                    <Input id="freight_distance" type="number" placeholder="Ex: 350" value={formData.freight_distance} onChange={e => handleChange("freight_distance", e.target.value)} />
                  </div>

                  <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="refrigerated" checked={formData.refrigerated} onCheckedChange={checked => handleChange("refrigerated", !!checked)} />
                      <Label htmlFor="refrigerated">Refrigerado</Label>
                    </div>
                    
                    <div className="flex items-center space-x-2">
                      <Checkbox id="requires_mopp" checked={formData.requires_mopp} onCheckedChange={checked => handleChange("requires_mopp", !!checked)} />
                      <Label htmlFor="requires_mopp">Requer MOPP</Label>
                    </div>
                    
                    <div className="flex items-center space-x-2">
                      <Checkbox id="toll_included" checked={formData.toll_included} onCheckedChange={checked => handleChange("toll_included", !!checked)} />
                      <Label htmlFor="toll_included">Pedágio Incluso</Label>
                    </div>

                    <div className="flex items-center space-x-2">
                      <Checkbox id="live_cargo" checked={formData.live_cargo} onCheckedChange={checked => handleChange("live_cargo", !!checked)} />
                      <Label htmlFor="live_cargo">Carga Viva</Label>
                    </div>

                    <div className="flex items-center space-x-2">
                      <Checkbox id="dry_cargo" checked={formData.dry_cargo} onCheckedChange={checked => handleChange("dry_cargo", !!checked)} />
                      <Label htmlFor="dry_cargo">Carga Seca</Label>
                    </div>

                    <div className="flex items-center space-x-2">
                      <Checkbox id="tarp_required" checked={formData.tarp_required} onCheckedChange={checked => handleChange("tarp_required", !!checked)} />
                      <Label htmlFor="tarp_required">Lona Obrigatória</Label>
                    </div>

                    <div className="flex items-center space-x-2">
                      <Checkbox id="has_insurance" checked={formData.has_insurance} onCheckedChange={checked => handleChange("has_insurance", !!checked)} />
                      <Label htmlFor="has_insurance">Com Seguro</Label>
                    </div>

                    <div className="flex items-center space-x-2">
                      <Checkbox id="has_tracker" checked={formData.has_tracker} onCheckedChange={checked => handleChange("has_tracker", !!checked)} />
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Textarea id="observations" placeholder="Descreva mais detalhes sobre a carga, como dimensões, requisitos específicos, etc." value={formData.observations} onChange={e => handleChange("observations", e.target.value)} rows={4} />
                  </div>
                </div>
